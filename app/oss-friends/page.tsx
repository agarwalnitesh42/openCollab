...
export default async () => {
  ...
  return (
    <section className="mt-24 max-w-6xl mx-auto px-4 md:px-8">
      <h1 className="text-black text-3xl font-semibold text-center">Our Open-source Friends</h1>
      <div className="mt-12 space-y-6 gap-4 grid-cols-2 sm:grid lg:grid-cols-3 sm:space-y-0">
        {data.map((item: OSSFriend, key: number) => (
          <Link
            ...
            key={key}
            target="_blank"
            className="flex flex-col no-underline group relative space-y-3 w-full border border-slate-800 rounded-lg p-5 bg-[linear-gradient(179.23deg,_#fff_0.66%,_rgba(30,_41,_59,_0)_255.99%)] hover:bg-slate-800 duration-200"
          >
            <h2 className="text-base text-black font-semibold">{item.name}</h2>
