...
export default () => {
  ...
  return (
    <footer className="mt-20 text-black bg-white px-4 py-5 max-w-screen-xl mx-auto md:px-8">
      <div className="border-t border-slate-800 pt-8">
        <div className="max-w-lg sm:mx-auto sm:text-center">
          <Brand className="sm:m-auto" />
          <p className="leading-relaxed mt-3 text-black text-[15px]">
        ...
        <ul className="text-sm font-medium items-center justify-center mt-8 space-y-5 sm:flex sm:space-x-4 sm:space-y-0">
          {footerNavs.map((item, idx) => (
            <li>
              <a key={idx} href={item.href} className="block hover:text-black">
                {item.name}
              </a>
            </li>
          ))}
        ...
        <div className="mt-8 items-center justify-between sm:flex">
          <div className="mt-4 sm:mt-0">
            ...
            <div className="text-xs pt-2">
              Built by{' '}
              {builtBy.map((t, i) => (
                <a className=" text-black hover:text-black" href={t.url} rel="nofollow noopener noreferrer" target="_blank">
                  {t.title}
                  {builtBy.length - 1 === i ? '' : ', '}
                </a>
              ))}
            ...
            <ul className="flex items-center space-x-4">
              <li>
                <a href="https://twitter.com/exp_nit">
                  <svg className="svg-icon w-6 h-6 text-black0 hover:text-black" viewBox="0 0 20 20">
                    <path
                      fill="none"
