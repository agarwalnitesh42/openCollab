...
const ArticleCard: React.FC<ArticleProps> = ({ article }) => {
  return (
    <li key={article.id} className="border-b border-slate-800 py-8">
      <div className="flex flex-wrap gap-2 items-center w-full text-sm text-black0">
        <span>
          Published{' '}
      ...
      <Link href={`/blog/${article.slug}`} className="block mt-2 mb-3 font-medium text-black hover:underline duration-150">
        <h2>{article.headline}</h2>
      </Link>
      <div className="text-black text-sm line-clamp-2  mb-4 block">{article.metaDescription}</div>
